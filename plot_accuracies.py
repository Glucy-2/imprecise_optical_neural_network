...
def compare_plot(
    accs, names, log_plot=False, f_name=None, q="Accuracy", acc_loss=False, colors=None
):
    if acc_loss:
    ...
    sigs = []
    for acc in accs:
        sigs.append(np.linspace(0, 0.02, acc.shape[1]))
    ...
    for n in range(len(accs)):
        ...
        plt.fill_between(sig, low, high, color=c, alpha=0.2)
...
def plot_psbs_acc(accuracies):
    ...
    cbar = plt.colorbar()
    ctr = plt.contour(
        uniform_filter(mean, 3), colors="white", extent=extent, linewidths=4
    )
    cbar.add_lines(ctr)
...
def plot_compare_fft_diff_nh():
    ...
    for f in files:
        ...
        accs.append(np.load(f_path))
        names.append(f"FFTNet, D={f[:-4]}")
...
if __name__ == "__main__":
    ...
    assert False
    acc_grid = np.load(F_GRID_ACC_DIAG)
    acc_stacked_fft = np.load(os.path.join(DIR_NOISY_TEST, "stacked_fft_diag.npy"))
    acc_layer_1 = np.load(os.path.join(DIR_NOISY_TEST, "grid_1_layer.npy"))
    acc_stacked_1 = np.load(os.path.join(DIR_NOISY_TEST, "stacked_fft_1.npy"))
    compare_plot(
        [acc_grid, acc_stacked_fft],
        ["GridNet", "StackedFFT-Net"],
        acc_loss=False,
        colors=["red", "green"],
    )
    ...
    acc_grid = np.array([np.diag(acc) for acc in acc_grid])
    acc_fft = np.load("./noisy_fft_93/accuracies.npy")
    acc_fft = np.array([np.diag(acc) for acc in acc_fft])
    ...
    compare_plot(
        ...
        "Grid_2",
        False,
        f_name="./figures/compare_grid.pdf",
        q="Accuracy",
    )
