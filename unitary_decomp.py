...
def null_element(U, i, j, backward=False):
    log.info(
        f'Nulling element U[{i}, {j}], {"backward" if backward else "forward"} stage'
    )
    N, _ = U.shape
    if backward:
        X = U[[i - 1, i], j]
    ...
    if np.abs(U[i, j]) > epsilon:
        log.warning(
            f"The element U[{i}, {j}] was not nulled within tolerance of {epsilon}, its abs value is {np.abs(U[i, j]):.2e}"
        )
    else:
