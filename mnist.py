...
def mnist_ONN(
    ...
    layers = [
        Linear(
            num_in, hidden_units[0], sigma_PS=sigma_PS, sigma_BS=sigma_BS, UNet=unitary
        ),
        ModNonlinearity(f=f),
    ]
...
def mnist_complex(
    num_in=N_IN, num_out=10, hidden_units=[256, 256], device=DEVICE, sigma=0, T0=0.03
):
    f = ShiftedSoftplus(T=T0)
    layers = [ComplexLinear(num_in, hidden_units[0]), ModNonlinearity(f=f)]
    for nh_, nh in zip(hidden_units[:-1], hidden_units[1:]):
        layers.extend(
            [
    ...
    layers.extend(
        [ComplexLinear(hidden_units[-1], num_out), ComplexNorm(), nn.LogSoftmax(dim=1)]
    )
    ...
    for epoch in range(n_epochs):
        for batch_idx, (data, target) in enumerate(loader):
            ...
            if batch_idx % log_interval == 0 and batch_idx != 0:
                ...
                out = model(data)
                print(
                    f"Epoch: {epoch}, Train loss: {loss.float():.4f}, Train acc: {acc:.4f}, Time/it: {t/log_interval * 1e3:.4f} ms"
                )
                if log_callback:
...
def mnist_grid_truncated(
    ...
    layers = [
        TruncatedGridLinear(
            num_in, hidden_units[0], sigma_PS=sigma_PS, sigma_BS=sigma_BS
        ),
        ModNonlinearity(f=f),
    ]
    for nh_, nh in zip(hidden_units[:-1], hidden_units[1:]):
    ...
    layers.extend(
        [
            TruncatedGridLinear(
                hidden_units[-1], num_out, sigma_PS=sigma_PS, sigma_BS=sigma_BS
            ),
            ComplexNorm(),
            nn.LogSoftmax(dim=1),
        ]
