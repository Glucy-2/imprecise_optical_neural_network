    ...
    def get_UV(self):
        ...
        if self.sigma_BS > 0:
            ...
            if self.approx_sigma_bs:
                ...
                d_UV_A = UV_MZ(
                    self.D, theta_A + pi, phi_A + pi, "A", BS_noise=self.noise_A
                )
                d_UV_B = UV_MZ(
                    self.D, theta_B + pi, phi_B + pi, "B", BS_noise=self.noise_B
                )

                UV_A = [
                    UV + 2**0.5 * self.sigma_BS * dUV
                    for (UV, dUV) in zip(UV_A, d_UV_A)
                ]
                UV_B = [
                    UV + 2**0.5 * self.sigma_BS * dUV
                    for (UV, dUV) in zip(UV_B, d_UV_B)
                ]
            else:
                UV_A = UV_MZ(
                    self.D,
                    ...
                    phi_A,
                    "A",
                    BS_noise=[self.sigma_BS * x for x in self.noise_A],
                    new=True,
                )
                UV_B = UV_MZ(
                    self.D,
                    ...
                    phi_B,
                    "B",
                    BS_noise=[self.sigma_BS * x for x in self.noise_B],
                    new=True,
                )
        else:
            UV_A = UV_MZ(self.D, theta_A, phi_A, "A")
    ...
    def get_UV(self):
        ...
        if self.sigma_BS > 0:
            ...
            if self.approx_sigma_bs:
                ...
                d_UV = UV_FFT(
                    self.theta + pi, self.phi + pi, BS_noise=[noise_U, noise_V]
                )
                UV = [
                    UV + 2**0.5 * self.sigma_BS * dUV for (UV, dUV) in zip(UV, d_UV)
                ]
            else:
                UV = UV_FFT(
                    self.theta,
                    self.phi,
                    BS_noise=[self.sigma_BS * noise_U, self.sigma_BS * noise_V],
                    new=True,
                )
        else:
            UV = UV_FFT(self.theta, self.phi)
...
class StackedFFTUnitary(nn.Sequential):
    def __init__(self, D, n_stack=None, sigma_PS=0, sigma_BS=0):
        if n_stack is None:
            ...
            n_stack = int(D // P)
        layers = [
            FFTUnitary(D, sigma_PS=sigma_PS, sigma_BS=sigma_BS) for _ in range(n_stack)
        ]
...
class HybridUnitary(Unitary):
    def forward(self, X):
        ...
        if self.sigma_BS > 0:
            ...
            UV_A = [
                UV + self.sigma_BS * noise_A * dUV for (UV, dUV) in zip(UV_A, d_UV_A)
            ]
            UV_B = [
                UV + self.sigma_BS * noise_B * dUV for (UV, dUV) in zip(UV_B, d_UV_B)
            ]
        ...
        for n in range(num_layers_total):
            ...
            if (
                (n % n_layers_btw == 0)
                and (n != 0)
                and (n <= n_fft_perms * n_layers_btw)
            ):
...
class Diagonal(nn.Module):
    ...
    def init_params(self):
        ...
        self.amp = Parameter(th.tensor(6.0))
...
class Linear(nn.Module):
    ...
    def __init__(
        self, D_in, D_out, sigma_PS=0, sigma_BS=0, UNet=Unitary, FFT_shaper=False
    ):
        super().__init__()
        self.D_in = D_in
        self.D_out = D_out
    ...
    def emul_M(self, M, numpy=False, rand_S=True):
        if self.UNet != Unitary:
            raise Exception(
                "Decomposition of arbitrary matrices is only supported with GridUnitary."
            )
        if not numpy:
            Do2, Di2 = M.shape
            assert (Do2 % 2 == 0) and (Di2 % 2 == 0)
